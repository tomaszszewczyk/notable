...
const Highlighter = {
  ...
  languagesAliases: {
    // language => language to use instead
    sh: "bash",
    shell: "bash",
    zsh: "bash",
  },
  ...
  initLanguage(language: string): boolean {
    // Loading needed languages dynamically, for performance and because WebPack complains about the included `loadLanguages` function //TODO: Add support for peerDependencies
    ...
    if (
      lang.require &&
      !_.castArray(lang.require).every(Highlighter.initLanguage)
    )
      return false;
