...
class Tagbox extends React.PureComponent<
  { onChange: Function; tags: string[]; className?: string },
  {}
> {
  $tagbox;
  ...
  componentDidMount() {
    ...
    if (this.props.onChange) {
      this.$tagbox.on("tagbox:change", () =>
        this.props.onChange(this.$tagbox.tagbox("get"))
      );
    }
  }
    ...
    return (
      <div className={`tagbox card-footer bordered ${className || ""}`}>
        ...
        <input
          ...
          className="tagbox-partial autogrow autofocus compact small"
        />
      </div>
    );
  }
