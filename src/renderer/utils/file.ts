...
const File = {
  ...
  stat: Storage._wrapAction(
    async (filePath: string): Promise<fs.Stats | undefined> => {
      try {
  ...
  rename: Storage._wrapAction(async (oldPath: string, newPath: string) => {
    try {
      ...
      return await File._handleError(e, newPath, File.rename, [
        oldPath,
        newPath,
      ]);
    }
  ...
  write: Storage._wrapAction(async (filePath: string, content: string) => {
    try {
      ...
      return await File._handleError(e, filePath, File.write, [
        filePath,
        content,
      ]);
    }
