...
class Sorting extends Container<SortingState, MainCTX> {
  ...
  toggleType = () => {
    const type =
      this.state.type === SortingTypes.ASC
        ? SortingTypes.DESC
        : SortingTypes.ASC;
  ...
  sort = (
    notes: NoteObj[],
    by: SortingBys = this.state.by,
    type: SortingTypes = this.state.type
  ): NoteObj[] => {
    const iteratees = {
    ...
    const iteratee = iteratees[by],
      ...
      pinnedSortedByType =
        type === SortingTypes.ASC ? pinnedSortedBy : pinnedSortedBy.reverse(),
      unpinnedSortedByType =
        type === SortingTypes.ASC
          ? unpinnedSortedBy
          : unpinnedSortedBy.reverse();
