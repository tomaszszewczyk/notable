...
const Attachment = ({ removeAttachment, attachment, name, openInApp }) => {
  ...
  return (
    <div
      className="attachment button list-item"
      data-filename={name}
      onClick={() => openInApp(attachment)}
    >
      <span className="title small">{name}</span>
      <i
        className="icon xxsmall actionable on-hover"
        onClick={(e) => {
          e.stopPropagation();
          removeAttachment(undefined, attachment || { fileName: name });
        }}
      >
        close
      </i>
